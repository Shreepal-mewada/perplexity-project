...
async function createChat(token, title = 'Test Chat') {
  try {
    const response = await axios.post(`${API_BASE}/chats/create`, {
      title
    }, {
      headers: {
        'Authorization': `Bearer ${token}`
...
async function sendMessage(token, chatId, message) {
  try {
    const response = await axios.post(`${API_BASE}/chats/message`, {
      chatId,
      message
    }, {
      headers: {
...
async function runIsolationTest() {
  ...
  const msgResult = await sendMessage(loginA.token, newChat.chatId, 'This is a secret message from User A');
  ...
  try {
    const accessTest = await axios.get(`${API_BASE}/chats/messages/${newChat.chatId}`, {
      headers: {
  ...
  const isolationWorking = chatsB.count === 0 || !chatsB.chats.some(c => c._id === newChat.chatId);
